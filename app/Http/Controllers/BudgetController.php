...
class BudgetController extends Controller
{
    ...
    public function checkStockBulkWithoutBudget(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                'date_event'        => 'required|date',
            ...
            $dateFrom = \Carbon\Carbon::parse($request->date_event);
            $days = $request->days;
            ...
            foreach ($request->products as $productInput) {
                $product = Product::with([
                    'productStock',
                    'comboItems.product.productStock'
                ])->find($productInput['id_product']);

                if (!$product) {
                    $results[] = [
                        'stock' => false,
                        'product' => null,
                        'id_product' => $productInput['id_product'],
                        'error' => 'Producto no encontrado',
                        'available_stock' => null,
                        'requested_quantity' => $productInput['quantity'],
                        'check_stock_combo' => null,
                    ];
                    $allHaveStock = false;
                    continue;
            ...
            return ApiResponse::create($message, 200, [
                ...
                'module' => 'budget',
                'endpoint' => 'Verificar stock masivo sin presupuesto',
            ]);
    ...
    public function checkPriceBulkWithoutBudget(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                'date_event'                => 'required|date',
            ...
            return ApiResponse::create($message, 200, [
                ...
                'module' => 'budget',
                'endpoint' => 'Verificar precios masivo sin presupuesto',
            ]);
